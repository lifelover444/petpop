...
- 每只导入宠物的 PetPop 扩展信息写入该宠物目录下的 `petpop.pet.json`。
- 应用级窗口位置写入 `%APPDATA%/PetPop/petpop.pet.json`。
- 全局应用设置写入 `%APPDATA%/PetPop/settings.json`，目前用于保存专注时长、休息时长和全局宠物缩放。
- Codex 状态桥文件为 `%APPDATA%/PetPop/codex-activity.json`，只读轮询，不解析 `~/.codex` 日志或 SQLite。
- 启动时允许从旧 `%APPDATA%/PetDesk` 迁移数据到 `%APPDATA%/PetPop`；迁移只能补齐缺失数据，不能覆盖用户已有的 PetPop 数据。
...
- 专注模式窗口必须：
  ...
  - 默认隐藏，由桌宠右键打开
  - 失焦后自动隐藏入口气泡
- 关闭控制台窗口时隐藏到系统托盘，不直接退出应用。
- 系统托盘至少提供显示控制台、显示桌宠、隐藏桌宠和退出。
- 常驻宠物窗口不能有灰色或浅色底板。
- 拖动宠物时必须按方向切换动作：
  - 向左拖动：`running-left`
  - 向右拖动：`running-right`
...
- 右键桌宠打开专注模式入口；专注气泡中的“打开桌面端”必须通过主窗口命令恢复控制台。
...
- 移除宠物只能删除 `%APPDATA%/PetPop/pets/<pet-id>/` 下的导入副本。
- 移除前需要用户确认。
- 不能删除 Codex 源目录、PetDex 源数据、仓库样例宠物或用户原始 zip/文件夹。
- 如果移除的是当前选中宠物，运行时应切换到剩余第一只宠物；没有剩余宠物时清空选择并回到 `idle`。
...
- 专注模式气泡可以显示计时、专注/休息时长设置、开始/暂停/继续/重置、打开桌面端等必要控件。
