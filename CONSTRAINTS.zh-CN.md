...
PetDesk 是一个 Windows 优先的轻量桌宠运行时，用来运行 Codex 兼容宠物。它把 Hatch Pet 的“一句话生成宠物”、Codex 自定义宠物包、PetDex 现成宠物图库和通用桌面桌宠体验连接起来。
...
Windows 上重建前如果 `petdesk.exe` 正在运行，先停止：
